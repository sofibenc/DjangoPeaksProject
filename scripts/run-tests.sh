...
docker exec peaks_web /bin/sh -c "pipenv run ./manage.py test --settings=peaks_project.peaks_project.settings.tests $TESTS_TO_RUN"

# You could add further commands on to the end of that command, eg:
# docker exec peaks_web /bin/sh -c "pipenv run ./manage.py test --settings=myproject.settings.tests $TESTS_TO_RUN ; pipenv run flake8"